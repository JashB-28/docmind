...
.main-sub    { font-size: 14px; color: #888; margin-bottom: 16px; }
.provider-badge { display: inline-block; background: #ede9ff; color: #4a3aaa; border-radius: 999px; padding: 6px 12px; font-size: 12px; font-weight: 500; margin-bottom: 24px; }
...
def get_active_model() -> str:
    if get_active_provider() == "ollama":
        return st.session_state.ollama_model_name
    return st.session_state.openai_model_name


def get_active_docs() -> list[str]:
    return st.session_state.docs_loaded.get(get_active_provider(), [])
...
with st.sidebar:
    ...
    st.radio(
        ...
        key="provider",
        horizontal=True,
        format_func=lambda provider: provider.title(),
    ...
    if st.session_state.provider == "openai":
        st.text_input(
            ...
            placeholder="sk-...",
            key="openai_api_key",
            help="Required only when you are using OpenAI models or OpenAI embeddings.",
        )
        st.selectbox(
            "OpenAI model",
            OPENAI_MODELS,
    ...
    st.info(f"Active provider: {active_provider.title()} | Model: {active_model}")
    ...
    if uploaded and st.button("⚡ Index Documents", use_container_width=True, type="primary"):
        if ensure_provider_ready():
            os.makedirs("data", exist_ok=True)
            names = []
            for file in uploaded:
                with open(os.path.join("data", file.name), "wb") as out:
                    out.write(file.read())
            ...
            with st.spinner(f"Indexing documents for the {active_provider.title()} tab..."):
                try:
                    ...
                    st.success(
                        f"Indexed {total} chunks for the {active_provider.title()} knowledge base."
                    )
...
st.markdown(
    '<div class="main-sub">Ask anything about your documents with citations, confidence scores, and memory.</div>',
    unsafe_allow_html=True,
)
st.markdown(
    f'<div class="provider-badge">{get_active_provider().title()} workspace · {get_active_model()}</div>',
    unsafe_allow_html=True,
)


if not get_active_docs() and st.session_state.mode != "summarize":
    st.markdown(
        """
    <div class="empty-state">
        <div class="empty-icon">📂</div>
        <div class="empty-title">No documents loaded for this provider yet</div>
        <div class="empty-sub">Use the OpenAI or Ollama tab in the sidebar, then upload PDFs and click <strong>Index Documents</strong>.</div>
    </div>
    """,
        unsafe_allow_html=True,
    )
...
if st.session_state.mode == "chat":
    ...
    if submitted and user_input.strip() and ensure_provider_ready():
        history_str = "\n".join(
            [
    ...
    st.caption("Ask the same question and see how two indexed documents answer it side by side.")

    active_docs = get_active_docs()
    if len(active_docs) < 2:
        st.warning("Upload and index at least 2 documents in the current provider tab to use Compare mode.")
    else:
        col_a, col_b = st.columns(2)
        with col_a:
            doc_a = st.selectbox("Document A", active_docs, key="doc_a")
        with col_b:
            doc_b = st.selectbox(
                "Document B",
                active_docs,
                index=min(1, len(active_docs) - 1),
                key="doc_b",
            )

        compare_q = st.text_input("Question", placeholder="e.g. What are the main conclusions?")

        if st.button("⚖️ Compare", type="primary") and compare_q.strip() and ensure_provider_ready():
            col1, col2 = st.columns(2)
            for col, doc_name in [(col1, doc_a), (col2, doc_b)]:
                with col:
                    ...
                    with st.spinner("Querying..."):
                        try:
                            ...
                            render_sources(result["sources"][:2])
                        except Exception as exc:
                            st.error(str(exc))

elif st.session_state.mode == "summarize":
    st.markdown("### 📋 Summarize Document")
    st.caption("Upload any PDF and generate a summary with the currently active provider tab.")
    ...
    summary_prompts = {
        ...
        "Bullet point overview": (
            ...
            "Aim for completeness and cover all major points."
        ),
        "ELI5 - Plain language": (
    ...
    if sum_file and st.button("📋 Generate Summary", type="primary") and ensure_provider_ready():
        ...
        with st.spinner("Reading and summarizing..."):
            try:
                ...
                text = "\n\n".join([page.page_content for page in pages[:20]])
                if len(text) > 12000:
